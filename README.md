...
- **npm run build**  执行这一步之前可以去config.js中配置是否将echarts、latex、yuml等等打包上去，默认是没有打包进去，默认包大小286k(极限最小体积)左右，打包完成在towxml目录下生成dist文件夹
...
| speed       | Number            | 每多少秒打印一个字符，值越小打印越快，单位是毫秒，默认 5 毫秒打印一个字符                                          |5|
